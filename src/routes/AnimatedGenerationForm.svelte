<script lang="ts">
  ...
  import type { AnimatedGenerationFormData } from "./GenerationFormData";
  ...
  async function onFormSubmitted(event: SubmitEvent & { currentTarget: EventTarget & HTMLFormElement }) {
    ...
    const speed = parseFloat(formData.get(FormName.Speed) as string);
    const looped = formData.get(FormName.Looped) === "on";
...
<form onsubmit={onFormSubmitted}>
  <Field.Set>
    <Field.Group>
      ...
      <Field.Field orientation="horizontal">
        <Checkbox class="cursor-pointer" id="looped" name={FormName.Looped} bind:checked={currentLooped} />
